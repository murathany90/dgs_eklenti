...
import { mapLoadFlowSettings } from './loadflow-settings-mapper.ts';
import { mapElmVac } from './elm-vac-mapper.ts';
import { mapBoundaries } from './boundary-mapper.ts';
import { mapSecondaryControllers } from './secondary-control-mapper.ts';
...
export function mapElectricalNetwork(document: DgsDocument, modelId: string, modelHash: string): ElectricalCanonicalNetwork {
  ...
  const internationalConnections = mapElmVac(ctx), boundaries = mapBoundaries(ctx);
  const secondaryControllers = mapSecondaryControllers(ctx, boundaries, generators);
  const controls = mapControls(ctx, generators, transformers, shunts);
  const loadFlowSettings = mapLoadFlowSettings(ctx);
  const coverage = modelCoverage(generators, transformers, controls);
  const incomplete = ctx.incompleteCount > 0;
  return { modelId, modelHash, scope: 'FULL', completeness: incomplete ? 'PARTIAL' : 'COMPLETE', buses, lines, transformers, generators, loads, shunts, seriesCompensators, externalGrids, switches, controls, internationalConnections, secondaryControllers, boundaries, loadFlowSettings,
