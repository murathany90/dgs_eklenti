...
class ElectricalTests(unittest.TestCase):
    ...
    def test_shunt_and_series_compensation(self):
        model = base(); line(model); load(model, 1, 20, 5)
        model["shunts"].append({"id": "S", "bus": "B1", "qMvarPerStep": -2,
            "steps": 3, "currentStep": 2, "inService": True})
        model["seriesCompensators"].append({"id": "C", "fromBus": "B0", "toBus": "B1", "xOhm": -1,
